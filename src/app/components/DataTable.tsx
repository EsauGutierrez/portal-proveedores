              ...
              {sortedData.map((item) => {
                ...
                return (
                  <React.Fragment key={item.folio}>
                    ...
                    {canExpand && isExpanded && ( <tr className="bg-gray-50"><td colSpan={7} className="p-0"><div className="p-4 pl-16"><h4 className="text-md font-semibold text-gray-700 mb-2">Detalle de Recepciones</h4><table className="w-full text-sm"><thead className="bg-gray-100 rounded-lg"><tr><th className="px-4 py-2 font-semibold text-left text-gray-600">Folio Recepción</th><th className="px-4 py-2 font-semibold text-left text-gray-600">Fecha</th><th className="px-4 py-2 font-semibold text-right text-gray-600">Cantidad</th><th className="px-4 py-2 font-semibold text-right text-gray-600">Subtotal</th><th className="px-4 py-2 font-semibold text-right text-gray-600">Total</th><th className="px-4 py-2 font-semibold text-center text-gray-600"></th><th className="px-4 py-2 font-semibold text-center text-gray-600"></th></tr></thead><tbody>{item.recepciones.map(recepcion => { const cantidadTotal = recepcion.articles.reduce((sum, article) => sum + article.quantity, 0); const subtotal = recepcion.articles.reduce((sum, article) => sum + parseFloat(article.subtotal), 0); const total = recepcion.articles.reduce((sum, article) => sum + parseFloat(article.total), 0); const recepcionConTotales = { ...recepcion, cantidadTotal, subtotal: formatCurrency(subtotal), total: formatCurrency(total) }; return (<tr key={recepcion.id} className="border-b border-gray-200 last:border-b-0"><td className="px-4 py-2 text-gray-600">{recepcion.folio}</td><td className="px-4 py-2 text-gray-600">{new Date(recepcion.fecha).toLocaleDateString('es-MX')}</td><td className="px-4 py-2 text-gray-600 text-right">{cantidadTotal} uds.</td><td className="px-4 py-2 text-gray-600 text-right">${formatCurrency(subtotal)}</td><td className="px-4 py-2 text-gray-800 font-semibold text-right">${formatCurrency(total)}</td><td className="px-4 py-2 text-center"><button onClick={(e) => { e.stopPropagation(); handleOpenDetailsModal(recepcion); }} className="bg-blue-500 hover:bg-blue-600 text-white text-xs font-bold py-1 px-3 rounded-md transition-colors duration-200">Ver detalles</button></td><td className="px-4 py-2 text-center"><button onClick={(e) => { e.stopPropagation(); handleOpenUploadModal(recepcionConTotales, item); }} className="bg-green-500 hover:bg-green-600 text-white text-xs font-bold py-1 px-3 rounded-md transition-colors duration-200">Subir factura</button></td></tr>);})}</tbody></table></div></td></tr>)}
                  </React.Fragment>
                );
              })}
            </tbody>
